...
import { useGetPhoneNumbersByUser } from "../../hooks/adminAssistants";
...
const UserPhone = () => {
  ...
  } = useGetPhoneNumbersByUser(userId);
    ...
    <div className=" overflow-hidden p-10">
              ...
              <tr>
                <td colSpan="4" className="px-6 py-4 text-center text-red-500 bg-red-50 m-h-5">
                  <div className="flex justify-center items-center">
                    ...
                    <span>Error fetching phone numbers. Please try again.</span>
                  </div>
                </td>
              </tr>
            )}
            ...
            {userAssistants && userAssistants.length === 0 && (
              <tr>
                <td colSpan="4" className="px-6 py-4 text-center text-gray-500 min-h-5">
                  <div className="flex justify-center items-center">
                    <FaExclamationTriangle className="mr-3" />
                    <span>No phone numbers found for this user.</span>
                  </div>
                </td>
              </tr>
            )}
            
            {userAssistants && userAssistants.length > 0 && userAssistants.map((number, index) => (
              <tr key={number.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
      ...
      {userAssistants && userAssistants.length > 0 && (
        <div className="px-5 py-3  pl-0 border-t border-gray-200 text-sm text-gray-500">
          Showing {userAssistants.length} phone number{userAssistants.length !== 1 ? 's' : ''}
        </div>
      )}
    </div>
  );
